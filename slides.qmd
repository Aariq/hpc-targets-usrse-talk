...
format: 
  uaz-revealjs:
    ...
    mermaid: 
      theme: forest
    logo: logo.png
    footer-logo-link: "https://datascience.cct.arizona.edu/"
filters: 
  - reveal-header
...
::: column
![](images/ood-screenshot.png){fig-alt="The form to configure an Open OnDemand instance of RStudio with fields for Cluster, R version, Queue, Run Time, and Core Count" fig-align="right"}
:::
:::::
...
![](images/renv-screenshot.png){fig-alt="Console output from the renv package on startup including steps for \"bootstrapping renv 1.0.8\" and a prompt reading \"would you like to run `renv::restore()` to restore the project library?\"" width="769"}
...
library(targets)
tar_source()                                               # <1>
tar_option_set(packages = c("dplyr", "ggplot2"))  # <2>
list(                                                      # <3>
  tar_target(file, "data.csv", format = "file"),           # <3>
  tar_target(data, read.csv(file)),                        # <3>
...
crew.cluster::crew_controller_slurm(
  workers = 5, #<1>
  slurm_partition = "standard", #<2>
  slurm_time_minutes = 1200, #<2>
  slurm_log_output = "logs/crew_log_%A.out", #<2>
  slurm_log_error = "logs/crew_log_%A.err", #<2>
  slurm_memory_gigabytes_per_cpu = 5, #<2>
  slurm_cpus_per_task = 2, #<2>
  script_lines = c( #<2>
    "#SBATCH --account kristinariemer",  #<2>
    "module load R" #<2>
  ), #<2>
  seconds_idle = 600 #<3>
)
...
::: notes
1.  5 R sessions are launched as SLURM jobs with wall-time of 1200 (in this example)
2.  Main process passes data and code to workers as tasks to complete
3.  After all tasks are completed (or if a task errors), SLURM jobs are cancelled by the main process.
:::
...
[![nanonext logo](https://shikokuchuo.net/nanonext/logo.png){alt="nanonext" style="vertical-align:middle;" width="75" height="88"}](https://shikokuchuo.net/nanonext/) `nanonext`, R bindings for [NNG](https://nng.nanomsg.org/) (Nanomsg Next Gen), which powers...

[![mirai logo](https://shikokuchuo.net/mirai/logo.png){style="vertical-align:middle" width="75"}](https://shikokuchuo.net/mirai/) `mirai`, a "minimalist async evaluation framework for R", which powers...

[![crew logo](https://raw.githubusercontent.com/wlandau/crew/refs/heads/main/man/figures/logo.png){style="vertical-align:middle" width="75"}](https://wlandau.github.io/crew/) `crew`, a unifying interface for creating distributed worker launchers
...
`crew.cluster` default is persistent SLURM jobs for workers, but can have them shut down after `seconds_idle` and then start new workers when needed.\
